...
export default function Setup() {
  ...
  return (
    <div className="min-h-screen bg-[#0D0F1A] text-white font-sans selection:bg-neon-teal/30 overflow-hidden relative">
      ...
      <div className="max-w-4xl mx-auto px-8 py-24 relative">
        ...
        <div className="mb-16 text-center">
          <div className="flex justify-center gap-3 mb-10">
            {[1, 2].map((s) => (
              <div 
                key={s} 
                className={`h-1.5 w-32 rounded-full transition-all duration-700 ${s <= step ? "bg-neon-teal shadow-[0_0_20px_rgba(0,255,200,0.6)]" : "bg-white/5"}`} 
              />
            ))}
          </div>
          <h2 className="text-5xl md:text-6xl font-black font-display mb-6 tracking-tight leading-none">
            {step === 1 ? <>Personalize Your <span className='text-neon-pink'>Experience</span></> : <>Analyze Your <span className='text-neon-teal'>Potential</span></>}
          </h2>
        ...
        >
          <div className="absolute top-0 right-0 p-12 opacity-[0.02] group-hover:opacity-[0.05] transition-opacity">
            {step === 1 ? <User className="w-40 h-40" /> : <FileText className="w-40 h-40" />}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10">
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 flex items-center gap-2">
                  <User className="w-3.5 h-3.5 text-neon-teal" /> Full Name
                </label>
                <input
                  type="text"
                  value={formData.name}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  className="w-full bg-white/[0.02] border border-white/10 rounded-2xl px-6 py-4 text-lg font-medium focus:outline-none focus:ring-2 focus:ring-neon-yellow/50 transition-all placeholder:text-neutral-800"
                  placeholder="John Doe"
                />
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 flex items-center gap-2">
                  <Briefcase className="w-3.5 h-3.5 text-neon-teal" /> Target Role
                </label>
                <div className="relative">
                  <select
                    value={formData.role}
                    onChange={(e) => setFormData({ ...formData, role: e.target.value })}
                    className="w-full bg-white/[0.02] border border-white/10 rounded-2xl px-6 py-4 text-lg font-medium focus:outline-none focus:ring-2 focus:ring-neon-yellow/50 transition-all appearance-none cursor-pointer"
                  >
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 flex items-center gap-2">
                  <Zap className="w-3.5 h-3.5 text-neon-pink" /> Experience
                </label>
                <div className="relative">
                  <select
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 flex items-center gap-2">
                  <Globe className="w-3.5 h-3.5 text-neon-lavender" /> Interview Language
                </label>
                <div className="relative">
                  <select
            ...
            <div className="space-y-10 relative z-10">
              <div
                ...
                className={`border-2 border-dashed rounded-[2.5rem] p-20 text-center transition-all cursor-pointer group/upload ${
                  isDragActive ? "border-neon-teal bg-neon-teal/5 shadow-[0_0_30px_rgba(0,255,200,0.1)]" : "border-white/10 hover:border-neon-pink/50 bg-white/[0.02]"
                }`}
              >
                <input {...getInputProps()} />
                <div className="w-24 h-24 bg-white/5 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-2xl group-hover/upload:scale-110 transition-transform duration-500">
                  <Upload className={cn("w-10 h-10 transition-colors duration-500", isDragActive ? "text-neon-teal" : "text-neutral-500 group-hover:text-neon-pink")} />
                </div>
                {formData.resume ? (
                  <div>
                    <p className="text-2xl font-black font-display text-white mb-3">{formData.resume.name}</p>
                    <p className="text-xs font-bold text-neutral-500 uppercase tracking-widest">Click or drag to replace your resume</p>
                  </div>
                ) : (
                  <div>
                    <p className="text-2xl font-black font-display text-white mb-3">Drop your resume here</p>
                    <p className="text-xs font-bold text-neutral-500 uppercase tracking-widest">PDF, DOC, DOCX up to 5MB</p>
                  </div>
                )}
              </div>

              {isAnalyzingResume && (
                <div className="flex items-center justify-center gap-4 text-neon-teal animate-pulse">
              ...
              {resumeAnalysis && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="p-8 bg-neon-teal/5 border border-neon-teal/10 rounded-[2rem] flex gap-6"
                >
                  <div className="w-12 h-12 bg-neon-teal/10 rounded-2xl flex items-center justify-center shrink-0">
                    <Sparkles className="w-6 h-6 text-neon-teal" />
                  </div>
                  <div>
                    <h4 className="text-[10px] font-black text-neon-teal uppercase tracking-[0.2em] mb-2">AI Resume Insight</h4>
                    <p className="text-sm text-neutral-400 leading-relaxed font-medium">{resumeAnalysis}</p>
                  </div>
                </motion.div>
              )}
            </div>
          ...
          <button
            onClick={handleNext}
            disabled={isSaving}
            className="neon-btn w-full mt-12 py-6 flex items-center justify-center gap-4 text-lg group active:scale-[0.98]"
          >
            {isSaving ? (
              <Loader2 className="w-6 h-6 animate-spin" />
            ) : (
